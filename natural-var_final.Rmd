...
all_dates3 <- read.delim(file = "C:\\Users\\jesu4036\\Documents\\GitHub\\wytham-tits-fly\\all_dates3.txt", sep = ",") %>% distinct()
...
climate <- read.delim(file = "C:\\Users\\jesu4036\\Documents\\GitHub\\wytham-tits-fly\\climate_filled.csv", sep = ',')

climate$datetime <- as.POSIXct(climate$datetime, format = "%Y-%m-%d %H:%M:%S")

caught.hour.day <- read.delim(file = "C:\\Users\\jesu4036\\Documents\\GitHub\\wytham-tits-fly\\caught.hour.day.csv", sep = ',')
...
#before
Anova(lmer(acc ~ species*Sex*age + headwind + (1|Ring), data = nat.var_mf), type = 3, test.statistic = "F")

summary(lmer(acc ~ species*Sex*age + headwind + (1|Ring), data = nat.var_mf), type = 3)

#after
Anova(lmer(acc ~ species*Sex*age + headwind + (1|Ring), data = nat.var_mf_ordinary), type = 3, test.statistic = "F")

summary(lmer(acc ~ species*Sex*age + headwind + (1|Ring), data = nat.var_mf_ordinary), type = 3)
