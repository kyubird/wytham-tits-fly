...
raw.flight.metrics <- t(raw.flight.metrics)
...
## A. Raw data graph of individual & species variance 
### v1
```{r}
...
## B. Non-significance of weather effects
### v1
#### headwind
```{r}
...
```
## C. v2 and avg.v ~ headwind
### v2 and avg.v model averaging
- tried log(response variable), but still same result of just headwind.
```{r}
set.seed(86)
summary(model.avg(dredge(lmer(v2 ~ species * Sex * age + headwind + (1|Ring), data = nat.var, na.action = "na.fail")), subset = delta <  10))
...
plot(nat.var$headwind, nat.var$avg.v)
```
#### plot accounting for individuals
```{r}

```
## D. Are interactions any significant?
#### v1
```{r}
Anova(lmer(v1 ~ species*Sex*age + headwind + (1|Ring), data = nat.var), type = 3, test.statistic = "F")
...
ggarrange(plot_v1_agesex, plot_acc_agesex, nrow =2 )
...
bw_species <- nat.var %>%
  ...
  summarise(
    ...
    max = max(Wt, na.rm = TRUE),
    min = min(Wt, na.rm = TRUE)
  )
bw_species
```